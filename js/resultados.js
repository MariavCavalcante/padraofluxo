 ...
 * pesquisadas (Seção 30) e da exportação (Seção 43).
 *
...
const Resultados = (() => {
  ...
  const COLUNAS_EXPORTACAO = [
    'Ordem', 'Distância (km)', 'Unidade Executante', 'Município Executante',
  ...
  function htmlDistancia(registro) {
    ...
    return `<span class="badge-distancia"><i class="fa-solid fa-location-dot"></i> ${registro.distanciaTexto} KM</span>`;
  }
  ...
  function exportarCsv() {
    if (ultimosResultados.length === 0) {
      Utilitarios.exibirAlerta('Não há resultados para exportar.', 'warning');
      return;
    }
    const linhas = ultimosResultados.map((r) => ({
      'Ordem': r.ordem,
      'Distância (km)': r.distanciaConhecida ? r.distanciaTexto : 'Não informada',
      'Unidade Executante': r.unidadeExecutante,
      'Município Executante': r.municipioExecutante || '',
      'Especialidade': r.especialidade,
      'Origem': textoOrigem(r)
    }));
    const csv = Utilitarios.paraCsv(linhas, COLUNAS_EXPORTACAO);
    Utilitarios.baixarArquivoTexto(`fluxo-padrao-resultado-${Date.now()}.csv`, csv);
  }
  ...
  function exportarExcel() {
    ...
    const linhas = ultimosResultados.map((r) => ({
      'Ordem': r.ordem,
      'Distância (km)': r.distanciaConhecida ? r.distanciaKm : 'Não informada',
      'Unidade Executante': r.unidadeExecutante,
      'Município Executante': r.municipioExecutante || '',
      'Especialidade': r.especialidade,
      'Origem': textoOrigem(r)
    }));
    const planilha = XLSX.utils.json_to_sheet(linhas, { header: COLUNAS_EXPORTACAO });
    const livro = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(livro, planilha, 'Resultado');
    XLSX.writeFile(livro, `fluxo-padrao-resultado-${Date.now()}.xlsx`);
  }

  function imprimir() {
    if (ultimosResultados.length === 0) {
      Utilitarios.exibirAlerta('Não há resultados para imprimir.', 'warning');
      return;
    }
    window.print();
  }
