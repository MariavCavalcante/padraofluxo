...
### 1.2 Tempo Estimado (min)

Desde 21/08/2026 a planilha real também traz uma coluna de Tempo Estimado
(em minutos) na aba DISTÂNCIA, ao lado da distância em KM. O painel agora
exibe esse valor — como "~XX min" — logo ao lado da distância, tanto na
tabela de resultados quanto no topo do "Ver Detalhes". É um dado
complementar e informativo: **nunca** é usado para ordenar os resultados
(a ordenação continua sendo estritamente por menor distância em KM). Se a
planilha não trouxer esse valor para algum par de municípios, ele
simplesmente não aparece — não gera erro nem impede a exibição da distância.

### 1.3 Tabela principal enxuta
...
└── data/                        # Cópia local de contingência das 3 abas (CSV), extraída em 21/08/2026
    ├── base_atualizada.csv      # snapshot mais recente da planilha (ver Seção 9 abaixo)
    ├── intermediacao.csv        # idem
    └── distancia.csv            # idem — inclui as colunas "Distância Atualizada (km)" e "Tempo Estimado (min)"
```
...
### 3.1 Nomes das colunas da aba DISTÂNCIA

Centralizados em `CONFIG.COLUNAS_DISTANCIA` (config.js). O nome da coluna de
distância já mudou uma vez na planilha real — de `"Distancia"` para
`"Distância Atualizada (km)"`, quando a equipe adicionou a coluna `"Tempo
Estimado (min)"` (21/08/2026). Se a planilha for reestruturada de novo,
ajuste apenas esse objeto em `config.js`; o código tolera tanto o nome atual
quanto o antigo (`"Distancia"`) como fallback. A coluna de Tempo Estimado já
está disponível na base e já é exibida na interface do painel (ver Seção 1.2).

### 3.2 Identificação de Município Executante (config.js)
...
**Achado da Qualidade da Base em 21/08/2026 (última reconexão):** o
município executante "Rio Verde" está sem nenhuma distância cadastrada na
aba DISTÂNCIA — aparecerá sempre como "Distância não informada" até que a
planilha seja corrigida. (O caso anterior, "Morrinhos", já foi resolvido
pela equipe na planilha.)
...
- Municípios Executantes sem nenhuma distância cadastrada (ex.: Rio Verde, na reconexão de 21/08/2026);
...
- [x] Carregamento online (última reconexão em 21/08/2026): 1.913 linhas de
      BASE_ATUALIZADA + 62 de INTERMEDIAÇÃO → 1.590 registros consolidados,
      63 unidades, 45 especialidades, 246 municípios solicitantes, zero
      erros de console
...
- [x] Ordenação sempre do menor para o maior KM; selo "MAIS PRÓXIMA" só no
      primeiro válido
- [x] Tempo Estimado (min) aparece ao lado da distância na tabela e no
      "Ver Detalhes", sem nunca influenciar a ordenação (sempre por menor KM)
- [x] Unidade sem distância cadastrada (Rio Verde) → "Distância não
